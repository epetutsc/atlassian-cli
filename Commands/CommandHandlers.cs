...
public static class CommandHandlers
{
    ...
    /// <param name="directContent">Direct content string (optional).</param>
    /// <param name="filePath">Path to a file containing the content (optional).</param>
    /// <param name="contentName">Name of the content for error messages (e.g., "description").</param>
    ...
    /// <exception cref="FileNotFoundException">Thrown when the specified file does not exist.</exception>
    private static string? ResolveOptionalContent(string? directContent, string? filePath, string contentName)
    {
        bool hasDirectContent = !string.IsNullOrEmpty(directContent);
        bool hasFilePath = !string.IsNullOrEmpty(filePath);
        ...
        if (hasDirectContent && hasFilePath)
        {
            throw new ArgumentException($"You cannot specify both --{contentName} and --{contentName}-file. Please use only one.");
        }
    ...
    public static async Task<int> HandleCreateIssueAsync(CreateIssueOptions options)
    {
        try
        {
            string? description = ResolveOptionalContent(options.Description, options.DescriptionFilePath, "description");
