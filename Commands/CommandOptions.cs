...
public class GetBambooBuildLogsOptions
{
    ...
    [Option('f', "filter", Required = false, Separator = ',', HelpText = "Filter log lines using regex patterns (case-insensitive). Multiple patterns can be specified separated by commas or by using -f multiple times. A line matches if ANY pattern matches. Examples: -f error -f exception or -f 'error,exception' or -f 'error|exception'")]
