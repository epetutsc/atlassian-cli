...
public static class Program
{
    ...
    private static void ShowBambooHelp()
    {
        ...
        Console.WriteLine("  atlassiancli bamboo get-build-logs --key PROJ-PLAN-123 --filter error");
        Console.WriteLine("  atlassiancli bamboo get-build-logs --key PROJ-PLAN-123 --filter exception");
        Console.WriteLine("  atlassiancli bamboo queue-build --key PROJ-PLAN");
    ...
    private static Task<int> HandleBambooParseErrorsAsync<T>(ParserResult<T> result, IEnumerable<Error> errors)
    {
        var helpText = HelpText.AutoBuild(result, h =>
        {
            ...
            h.AddPreOptionsLine("  get-build-logs     Retrieve build logs (with optional filter)");
            ...
            h.AddPreOptionsLine("  atlassiancli bamboo get-build-logs --key PROJ-PLAN-123 --filter error");
